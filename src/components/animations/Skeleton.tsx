...
export function SkeletonCard({
  ...
  return (
    <div className={`bg-bg-600 border-bg-200 rounded-xl border p-4 ${className}`}>
      {hasImage && <Skeleton variant="rounded" height={120} className="mb-4" />}
      {hasTitle && <Skeleton variant="text" height={20} width="60%" className="mb-3" />}
...
export function SkeletonAvatar({
  ...
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      ...
      {showName && <Skeleton variant="text" width={80} height={16} />}
    </div>
  );
}
