...
function ToastItem({ toast, onRemove }: { toast: ToastData; onRemove: (id: string) => void }) {
  ...
  const typeStyles: Record<
    ToastType,
    { bg: string; icon: string; iconColor: string; progress: string }
  > = {
    success: {
  ...
  return (
    <div
      ref={toastRef}
      className={`relative overflow-hidden rounded-xl border ${style.bg} max-w-[400px] min-w-[320px] shadow-2xl backdrop-blur-lg ${
    ...
    >
      <div className="flex items-start gap-3 p-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          {toast.message && (
            <p className="mt-0.5 line-clamp-2 text-sm text-gray-400">{toast.message}</p>
          )}
        </div>
      ...
      {duration > 0 && (
        <div className="absolute right-0 bottom-0 left-0 h-1 bg-white/5">
          <div ref={progressRef} className={`h-full origin-left ${style.progress}`} />
        </div>
      )}
    </div>
  );
...
export function ToastContainer({
  ...
  return (
    <div className="fixed right-6 bottom-6 z-100 flex flex-col-reverse gap-3">
      {toasts.map((toast) => (
